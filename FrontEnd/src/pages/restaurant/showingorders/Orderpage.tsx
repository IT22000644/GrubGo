...
const StatusBadge: React.FC<{ status: Order['status'] }> = ({ status }) => {
    ...
    return (
        <span className={`${getStatusStyle()} text-xs font-medium px-3 py-1 rounded-full border`}>
            {status.toUpperCase()}
        </span>
    );
};
...
const OrderPage: React.FC = () => {
    ...
    const activeOrders = orders.filter(order => order.status !== 'pending' && !order.iscompleted);
                            ...
                            <div className="overflow-x-auto pb-4 -mx-4 px-4">
                                <div className="flex space-x-4 min-w-max pb-2">
                                    {completedOrders.map(order => (
                                        <div key={order._id} className="min-w-[280px] max-w-xs flex-shrink-0">
                                            <OrderCard order={order} onStatusChange={handleStatusChange} compact={true} />
                                        </div>
                                    ))}
                                </div>
                            </div>
